...
public class powxn {
    public double myPow(double x, int Y) {
        ...
        if (y == 2.0) {
            ...
            if (x >= -Double.MAX_VALUE) // Handle x == -Double.POSITIVE_INFINITY later
                return Math.sqrt(x + 0.0); // Add 0.0 to properly handle x == -0.0
        } else if (y_abs == 1.0) {        // y is  +/-1
            return (y == 1.0) ? x : 1.0 / x;
        } else if (y_abs == Double.POSITIVE_INFINITY) {       // y is +/-Double.POSITIVE_INFINITY
            if (x_abs == 1.0)
        ...
        // Special value of x
        if (x_abs == 0.0 ||
                x_abs == Double.POSITIVE_INFINITY ||
                x_abs == 1.0) {
        ...
        if (j >= 0x40900000) {                           // z >= 1024
            if (((j - 0x40900000) | i)!=0)               // if z > 1024
                return s * Double.POSITIVE_INFINITY;                     // Overflow
            else {
