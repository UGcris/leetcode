...
public class SudokuSolver {
    public void solveSudoku(char[][] board) {
        int[][] rowv=new int[9][9];
        int[][] colv=new int[9][9];
        int[][] squv=new int[9][9];

        int nilSize=0;
        int[][] nilv=new int[81][2];
        for(int x=0;x<9;x++){
            for(int y=0;y<9;y++){
                if('.'==board[x][y]){
                    nilv[nilSize]=new int[]{x,y};
                    nilSize++;
                    continue;
                }
                int idx=board[x][y]-'1';
                rowv[x][idx]=1;
                colv[y][idx]=1;
                squv[getsquidx(x,y)][idx]=1;
            }
        }
        solveSudoku(board,rowv,colv,squv,nilv,nilSize);
    }
